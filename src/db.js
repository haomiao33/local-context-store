...
export function openDatabase(filename) {
  ...
  try {
    // Set the busy timeout before WAL/schema initialization so startup itself
    // is resilient when several agent processes open the same project at once.
    db.pragma('busy_timeout = 5000');
    withSqliteRetry(() => db.pragma('journal_mode = WAL'));
    db.pragma('foreign_keys = ON');
    withSqliteRetry(() => db.exec(`
